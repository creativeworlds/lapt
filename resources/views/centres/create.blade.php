<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white p-6 shadow-md rounded-lg">
            <form action="{{ route('centres.store') }}" method="post" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" name="code" id="code" value="{{ old('code') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    @error('code')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
